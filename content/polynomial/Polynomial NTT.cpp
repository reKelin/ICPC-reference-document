...
#define T (*this)
    ...
    Poly mulT(const Poly& b) const { return T * b.rev() >> (b.deg() - 1); }
    ...
    Poly exp2(int n) const {
        ...
        for (int m = 1; m < n; m++) {
            ...
            Poly &p = va[k], q(b.begin() + l, b.begin() + m);
            if (p.empty()) p = a.pre(r - l - 1), p.resize(k * 2), dft(p);
            q.resize(k * 2), dft(q), idft(q ^= p);
            for (int i = m; i < r; i++) inc(b[i], q[i - l - 1]);
            b[m] = mul(b[m], ::inv[m]);
        }
        return b;
    }
