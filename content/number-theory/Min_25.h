...
/*
 * Description: Calculate sum f(x), where f(x) is a  multiplicative function,
 *   and f(p) = Poly(p), and f(p ^ k) can be calculated quickly.
 * Time: O(n ^ 0.75 / log n)
 */
...
// modify  function
struct fun {
    ll g(int p) { return p; }                                // f(p) = poly(p) = sum a_i * g_i(p), g_i(p) = p ^ i
    ll G(ll n) { return n %= P, (n * (n + 1) / 2 - 1) % P; } // G(n) = sum g - g(1)
    ll f(int p, int k, ll pk) { return k == 1 ? p : 0; }     // f(p ^ k) can be calculated quickly
} Fun;
